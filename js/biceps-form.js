...
function downloadWorkoutCSV(formData) {
    ...
    link.href = url; link.download = `bicep_workout_${new Date().toISOString().slice(0,10)}.csv`;
    document.body.appendChild(link); link.click(); document.body.removeChild(link);
}

function computeAllScores(repData) {
    if (!repData?.length) return { finalScore: 0, avgScores: { timeScore: 0.8, romScore: 0.8, formScore: 0.8, momentumScore: 0.8 } };
    
    const sums = { timeScore: 0, romScore: 0, formScore: 0, momentumScore: 0 };
    repData.forEach(rep => {
        const scores = window.computeRepScores?.(rep) || { timeScore: 0.8, romScore: 0.8, formScore: 0.8, momentumScore: 0.8 };
        Object.keys(sums).forEach(key => sums[key] += scores[key] || 0.8);
    });
    
    const n = repData.length;
    const avgScores = Object.fromEntries(Object.keys(sums).map(k => [k, sums[k] / n]));
    return { finalScore: (Object.values(avgScores).reduce((a,b)=>a+b,0) / 4) * 100, avgScores };
}
